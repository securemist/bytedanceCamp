...
func main() {
	...
	douyin_extra_first.RegisterCommentServer(server, &service.CommentService{})
	...
	err = registerClient.Register(
		...
		global.ProjectConfig.ConsulService.Comment.Tags,
		ServiceId,
	)
	if err != nil {
		zap.S().Errorf("注册%s服务失败: %s", global.ProjectConfig.ConsulService.Comment.Name, err.Error())
	} else {
		zap.S().Infof("注册%s服务成功: %s:%d", global.ProjectConfig.ConsulService.Comment.Name, "127.0.0.1", port)
	}
	// 优雅退出
	...
	if err != nil {
		zap.S().Errorf("注销%s服务失败: %s", global.ProjectConfig.ConsulService.Comment.Name, err.Error())
	} else {
		zap.S().Infof("注销%s服务成功: %s:%d", global.ProjectConfig.ConsulService.Comment.Name, "127.0.0.1", port)
	}
}
