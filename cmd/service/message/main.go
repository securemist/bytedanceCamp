...
func main() {
	...
	douyin_extra_second.RegisterMessageServer(server, &service.MessageService{})
	...
	err = registerClient.Register(
		"127.0.0.1",
		port,
		global.ProjectConfig.ConsulService.Message.Name,
		global.ProjectConfig.ConsulService.Message.Tags,
		ServiceId,
	)
	if err != nil {
		zap.S().Errorf("注册%s服务失败: %s", global.ProjectConfig.ConsulService.Message.Name, err.Error())
	} else {
		zap.S().Infof("注册%s服务成功: %s:%d", global.ProjectConfig.ConsulService.Message.Name, "127.0.0.1", port)
	}
	// 优雅退出
	...
	if err != nil {
		zap.S().Errorf("注销%s服务失败: %s", global.ProjectConfig.ConsulService.Message.Name, err.Error())
	} else {
		zap.S().Infof("注销%s服务成功: %s:%d", global.ProjectConfig.ConsulService.Message.Name, "127.0.0.1", port)
	}
}
