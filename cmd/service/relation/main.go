...
func main() {
	...
	douyin_extra_second.RegisterRelationServer(server, &service.RelationService{})
	...
	err = registerClient.Register(
		...
		global.ProjectConfig.ConsulService.Relation.Tags,
		ServiceId,
	)
	if err != nil {
		zap.S().Errorf("注册%s服务失败: %s", global.ProjectConfig.ConsulService.Relation.Name, err.Error())
	} else {
		zap.S().Infof("注册%s服务成功: %s:%d", global.ProjectConfig.ConsulService.Relation.Name, "127.0.0.1", port)
	}
	// 优雅退出
	...
	if err != nil {
		zap.S().Errorf("注销%s服务失败: %s", global.ProjectConfig.ConsulService.Relation.Name, err.Error())
	} else {
		zap.S().Infof("注销%s服务成功: %s:%d", global.ProjectConfig.ConsulService.Relation.Name, "127.0.0.1", port)
	}
}
